...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.InterPolationOrderSlider.valueChanged.connect(lambda : self.Interpolate())
        ...
        self.LinearInterpRadioBtn.toggled.connect(lambda: self.Interpolate())
        self.PolynomialInterpRadioBtn.toggled.connect(lambda : self.Interpolate())
        ...
        pg.PlotItem.hideAxis(self.CurveFittingGraph, 'left')
        pg.PlotItem.hideAxis(self.CurveFittingGraph, 'right')
        pg.PlotItem.hideAxis(self.CurveFittingGraph, 'bottom')
        self.dataCanvas = SignalCanvas(self, width=5, height=4, dpi=100)                       # Create canvas object
    ...
    def read_data(self,file_name):
        ...
        self.feature=self.feature[0:1000:1]
        self.target=list(pd.to_numeric(dataFile.iloc[:,-1],downcast="float"))
        self.target=self.target[0:1000:1]
    ...
    def createChunks(self):
        self.chunk_size = int(1000/self.numChunks)
        self.feature_chunks=[self.feature[i:i+self.chunk_size] for i in range(0, int(len(self.feature)), int(self.chunk_size))]
        self.target_chunks= [self.target[i:i+self.chunk_size] for i in range(0, int(len(self.target)), int(self.chunk_size))]
    def fitChunks(self):
        self.interpolatedTarget = []
        for i in range(0,self.numChunks,1):
            coeff = (np.polyfit(self.feature_chunks[i] , self.target_chunks[i] ,deg =self.InterpolationOrder))
            self.chunkCoeff.append(coeff)
            polynomial = np.poly1d(coeff)
            self.interpolatedTarget = np.concatenate((self.interpolatedTarget , polynomial(self.feature_chunks[i])))
    def plotData(self):
        self.signalYMin=min(self.target)
        self.signalYMax=max(self.target)
        self.signalXMin=min(self.feature)
        self.signalXMax=max(self.feature)
        self.settingCurveLimits()
        print(len(self.interpolatedTarget))
        self.dataCanvas.axes.clear()
        self.dataCanvas.axes.plot(self.feature,self.target) 
        if len(self.feature)==len(self.interpolatedTarget):
            self.dataCanvas.axes.scatter(self.feature , self.interpolatedTarget , marker = "." , c = 'red')   
        self.dataCanvas.draw()
        self.CurveFittingGraph.setCentralItem(self.graph)
        self.CurveFittingGraph.setLayout(self.layout)
    ...
    #         print(self.polyVectors)
    #         polynomial= np.poly1d(coeff)
    #         self.CurveFittingGraph.plot(self.feature[(self.Chunkorder-1)*int(1000/self.numChunks):self.index+int(1000/self.numChunks)],polynomial(self.feature[(self.Chunkorder-1)*int(1000/self.numChunks):self.index+int(1000/self.numChunks)]), pen=None ,symbol='+')
    ...
    #        print(self.polyVectors)
    #        polynomial= np.poly1d(coeff) 
    #        self.CurveFittingGraph.plot(self.feature[(self.Chunkorder-1)*int(1000/self.numChunks):(self.Chunkorder-1)*int(1000/self.numChunks)+int(1000/self.numChunks)],polynomial(self.feature[(self.Chunkorder-1)*int(1000/self.numChunks):(self.Chunkorder-1)*int(1000/self.numChunks)+int(1000/self.numChunks)]), pen=None ,symbol='+')
