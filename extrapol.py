...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.PolynomialInterpRadioBtn.toggled.connect(lambda : self.InterpolationOrdersetting(self.InterPolationOrderSlider.value()))
        ...
        self.p=self.CurveFittingGraph 
        self.original_curve = self.p.plot()
        self.interpolated_curve = self.p.plot()

        #setting canvas
        self.graph = pg.PlotItem()
        pg.PlotItem.hideAxis(self.p, 'left')
        pg.PlotItem.hideAxis(self.p, 'right')
        pg.PlotItem.hideAxis(self.p, 'bottom')
    ...
    def fitChunks(self):
        ...
        for i in range(0,self.numChunks,1):
            coeff,residual , a ,b ,c= np.polyfit(self.feature_chunks[i] , self.target_chunks[i] ,deg =self.InterpolationOrder , full = True)
            self.chunkCoeff.append(coeff)
            self.write_error(residual)
            if self.InterpolationOrder==1:
                self.linear_latex_equation(coeff)
            else:
                self.polynomial_latex_equation(coeff)
            polynomial = np.poly1d(coeff)
            # self.ExtrapolationFunc(coeff)   
            self.x_axis=self.feature[0:self.maxLength]
            self.y_axis = np.concatenate((self.y_axis , polynomial(self.feature_chunks[i])))
    def plotData(self):
        self.signalYMin=min(self.target)
        self.signalYMax=max(self.target)
        self.signalXMin=min(self.feature)
        ...
        if len(self.feature)==len(self.y_axis):
            self.dataCanvas.axes.scatter(self.feature , self.y_axis , marker = "." , c = 'red')   
        self.dataCanvas.draw()
        self.p.setCentralItem(self.graph)
        self.p.setLayout(self.layout)
    ...
    def ExtrapolationFunc(self, coeff):
        if self.numChunks != 1:
            ...
            if self.ExtrapolationCoef ==100:
                ...
                self.dataCanvas.axes.plot(self.x_axis,self.y_axis , c='#88c999' )
                print(polynomial)
