    ...
    def linearInterpolate(self):
        if self.ExtrapolationCoef ==0:
            ...
            if self.numChunks==1:
                ...
                coeff,residual,a,b,c=np.polyfit(self.feature[(self.Chunkorder-1)*int(maxLength/self.numChunks) : (self.Chunkorder*int(maxLength/self.numChunks))-1 ] , self.target[(self.Chunkorder-1)*int(maxLength/self.numChunks) : (self.Chunkorder*int(maxLength/self.numChunks))-1]  ,deg=1,full=True)
                polynomial= np.poly1d(coeff)      
    ...
    def polyInterpolate(self):
        if self.ExtrapolationCoef ==0:
            ...
            if self.numChunks==1:
                ...
                coeff,residual,a,b,c=np.polyfit(self.feature[(self.Chunkorder-1)*int(maxLength/self.numChunks) : (self.Chunkorder*int(maxLength/self.numChunks))-1 ] , self.target[(self.Chunkorder-1)*int(maxLength/self.numChunks) : (self.Chunkorder*int(maxLength/self.numChunks))-1] ,deg = self.InterpolationOrder,full=True)
                polynomial= np.poly1d(coeff) 
