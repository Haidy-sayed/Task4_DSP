...
#        self.CurveFittingGraph.
    ...
    def errorMap(self, errorVals):
      ...
      #  self.axes=[]
       # self.axes.append(self.numChunks)
        ...
        plt.savefig('../fig.png')
        #exporter = pyqtgraph.exporters.ImageExporter(plt.scene())
        #exporter.export('err.png') 
        #plt.colorbar()
        #errFig.tight_layout()
        #errFig.savefig('errFig.png', dpi=200) 
        plt.show()
        plt.close()
        ...
        if en==1:
            if self.ErrorMapXaxis =="Chunks":
                if self.ErrorMapYaxis=="Inter":
                    self.errorMappingCalcChunk(self.y_axis, self.feature, 0,1)
                else :
                    self.errorMappingCalcChunk(self.y_axis,self.feature,0,2)
            elif self.ErrorMapXaxis =="Inter":
                if self.ErrorMapYaxis=="Chunks":
                    self.errorMappingCalcChunk(self.y_axis, self.feature, 1,0)
                else :
                    self.errorMappingCalcChunk(self.y_axis,self.feature,1,2)
    ...
    def errorMappingCalcChunk(self, calculatedChunk, actaulChunk, axEnable,ayEnable):
        ...
        if(axEnable ==0):
            ...
            progressBarfixed=1
            #print(progressBarfixed)
            print(len(calculatedChunk))
            for j in range(self.numChunks):
                for iterator in range(len(calculatedChunk)):
                    ...
                    self.ErrorMappingProgressBar.setValue(progressBarfixed)
                    progressBarfixed+=1
                    sleep(0.005)
