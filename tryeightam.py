...
class Ui_MainWindow(object):
    ...
    def ploterrormap(self, x, y):
        ...
        plt.xlabel(x)
        plt.ylabel(y)
        #self.ErrorMappingCurve.draw_idle()
    ...
    def errorMappingRun(self,text):
        if text == 1:
            ...
            if self.ErrorMapXaxis == "Chunks" and self.ErrorMapYaxis == "Inter":
                if self.interpolationTypeFlag == 0 and self.numChunks ==1:
                    ...
                    for i in range (self.numChunks):
                        ...
                        #    self.ErrorMappingGraph.setItem(i,j,QTableWidgetItem(mpl.pyplot.show(self.result)))
                            #self.ErrorMappingGraph.item(i,j).setBackground(QtGui.QColor(self.color*1,self.color*1,self.color*10))
                            self.result=numpy.linalg.norm(self.target[1:999])-numpy.linalg.norm(self.y_axis)/(self.target[1:999])
                        self.newArray.append(self.result)
                    ...
                    #self.ploterrormap(self.x_axis,self.result)
                    ...
                    for i in range(1,self.numChunks+1,1):
                        ...
                        for j in range (self.InterPolationOrderSlider.value()):
                                ...
                                self.result=numpy.linalg.norm(self.target[self.fixedOverLapValue+ len(self.x_axis)*(i-1):self.fixedOverLapValue+ len(self.x_axis)*i if i < self.numChunks+1 else 1000])-numpy.linalg.norm(self.y_axis[self.fixedOverLapValue+ len(self.x_axis)*(i-1):self.fixedOverLapValue+ len(self.x_axis)*i if i < self.numChunks-1 else 1000])/self.target[self.fixedOverLapValue+ len(self.x_axis)*(i-1):self.fixedOverLapValue+ len(self.x_axis)*i if i < self.numChunks-1 else 1000]
                    ...
                    self.result=numpy.linalg.norm(self.target)-numpy.linalg.norm(self.y_axis)/self.target
                    #print(self.result)
                    self.color=int(self.result)
                    self.progressBarValue=int(100/self.numChunks)
                    for i in range (self.numChunks):
            ...
            self.ErrorMappingButton.setText("Run EM")
            print("Inside Error calc , before calling plotting error!!!")
            self.ploterrormap(self.x_axis, self.result)
